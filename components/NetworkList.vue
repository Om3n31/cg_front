<template>
    <div class="grid grid-cols-4 gap-5 mt-6">
        ...
        <div v-if="!loading" v-for="network in networks">
...
<script async setup lang="ts">
    ...
    setTimeout(async () => {
        const { data:networksData } = await useAsyncData(() => $fetch('http://localhost:8000/neuralnetwork/?format=json'));

        networks = ref(networksData.value)
